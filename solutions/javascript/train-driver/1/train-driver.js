...
export function fixListOfWagons(ids) {
  const [first, second, ...remaningIds] = ids;
  return [...remaningIds, first, second];
}
...
export function correctListOfWagons(ids, missingWagons) {
  const [first, ...remaningIds] = ids;
  return [first, ...missingWagons, ...remaningIds]
}
...
export function separateTimeOfArrival(information) {
  ...
  delete info.timeOfArrival;
  return[information.timeOfArrival, {...info}]
}
